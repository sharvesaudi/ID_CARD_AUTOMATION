...
driver.get('http://timf.chennairfid.com/admin/')

# Login with admin
passs = driver.find_element("name", "admin_password")
passs.send_keys("timf024")
...
for i in range(r):
    ...
    ele4 = driver.find_element("name", "alt_id")
    ele4.send_keys(df.iloc[i][3]) 
    
    ele5 = driver.find_element("name", "pan_number")
    ele5.send_keys(str(df.iloc[i][4]))
 
    ele6 = driver.find_element("name", "present_address")
    ele6.send_keys(df.iloc[i][5])

    ele7 = driver.find_element("name", "permanent_address")
    ele7.send_keys(df.iloc[i][6]) 

    ele8 = driver.find_element("name", "blood_group")
    ele8.send_keys(df.iloc[i][7]) 

    ele9 = driver.find_element("name", "contact_person")
    ele9.send_keys(df.iloc[i][8]) 

    ele10 = driver.find_element("name", "contact_number")
    ...
    ele12.send_keys(df.iloc[i][11])  

    ele13 = driver.find_element("name", "issue_date")
    ele13.send_keys(df.iloc[i][12])  

    # eles = driver.find_element("name", "submit")
    # eles.click()
